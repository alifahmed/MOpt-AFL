...
double x_now[swarm_num][operator_num], L_best[swarm_num][operator_num],
		...
		probability_now[swarm_num][operator_num], swarm_fitness[swarm_num];
...
static s32 shm_val_id; /* ID of the SHM region             */
static s32 shm_idx_id; /* ID of the SHM region             */
...
struct queue_entry {
	...
	u32 trace_mini_size; /* Trace bytes, if kept             */
...
[0] = 0, [1] = 1, [2] = 2, [3] = 4, [4 ... 7] = 8, [8 ... 15] = 16, [16 ... 31
		] = 32, [32 ... 127] = 64, [128 ... 255] = 128
...
static void remove_shm(void) {
	...
	if(shm_idx_id) shmctl(shm_idx_id, IPC_RMID, NULL);
	if(shm_val_id) shmctl(shm_val_id, IPC_RMID, NULL);
...
EXP_ST void setup_shm(void) {
	...
	shm_val_id = shmget(IPC_PRIVATE, MAP_SIZE, IPC_CREAT | IPC_EXCL | 0600);
	if (shm_val_id < 0)
		PFATAL("shmget() failed");
	atexit(remove_shm);
	shm_str = alloc_printf("%d", shm_val_id);
	if (!dumb_mode)
		setenv(SHM_VAL_ENV_VAR, shm_str, 1);
	ck_free(shm_str);

	shm_idx_id = shmget(IPC_PRIVATE, MAP_SIZE * 4, IPC_CREAT | IPC_EXCL | 0600);
	if (shm_idx_id < 0)
		PFATAL("shmget() failed");
	shm_str = alloc_printf("%d", shm_idx_id);
	if (!dumb_mode)
		setenv(SHM_IDX_ENV_VAR, shm_str, 1);
	ck_free(shm_str);

	trace_bits = shmat(shm_val_id, NULL, 0);
	if (!trace_bits)
			PFATAL("shmat() failed");
		...
		*rptr = 0;
		...
		while (*lptr) {
			...
			case '\\':
				...
				*(wptr++) = ((strchr(hexdigits, tolower(lptr[1])) - hexdigits)
						<< 4)
...
#  define CASE_PREFIX "id_"
		...
		if (!strncmp(rsl, CASE_PREFIX, 3)
			...
			if (use_name)
				...
				use_name = rsl;
			nfn = alloc_printf("%s/queue/id:%06u,orig:%s", out_dir, id,
					use_name);
...
static void write_stats_file(double bitmap_cvg, double stability, double eps) {
	...
	"command_line      : %s\n", start_time / 1000, get_cur_time() / 1000,
			getpid(), queue_cycle ? (queue_cycle - 1) : 0, total_execs, eps,
			queued_paths, queued_favored, queued_discovered, queued_imported,
			max_depth, current_entry, pending_favored, pending_not_fuzzed,
			queued_variable, stability, bitmap_cvg, unique_crashes,
			unique_hangs, last_path_time / 1000, last_crash_time / 1000,
			last_hang_time / 1000, total_execs - last_crash_execs, exec_tmout,
			use_banner, qemu_mode ? "qemu " : "", dumb_mode ? " dumb " : "",
	...
	if (limit_time_sig == 1) {
		if (key_puppet == 0) {
			if (key_module == 0) {
				...
				cYEL "MOpt-AFL (pilot_fuzzing)", use_banner);
			} else if (key_module == 1) {
				sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
				" (%s)", crash_mode ? cPIN "peruvian were-rabbit" :
				cYEL "MOpt-AFL (core_fuzzing)", use_banner);
			} else if (key_module == 2) {
				sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
				" (%s)", crash_mode ? cPIN "peruvian were-rabbit" :
				cYEL "MOpt-AFL (pso_updating)", use_banner);
			}
		} else {
			...
			if (key_module == 0) {
				sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
				" (%s)", crash_mode ? cPIN "peruvian were-rabbit" :
				cYEL "MOpt-AFL + pacemaker (pilot_fuzzing)", use_banner);
			} else if (key_module == 1) {
				sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
				" (%s)", crash_mode ? cPIN "peruvian were-rabbit" :
				cYEL "MOpt-AFL + pacemaker (core_fuzzing)", use_banner);
			} else if (key_module == 2) {
				sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
				" (%s)", crash_mode ? cPIN "peruvian were-rabbit" :
				cYEL "MOpt-AFL + pacemaker (pso_updating)", use_banner);
			}
		}
	} else {
		sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
...
#define SP20    SP10 SP10
	...
	SAYF(
			bV bSTOP "        run time : " cRST "%-34s " bSTG bV bSTOP "  cycles done : %s%-5s  " bSTG bV "\n",
	...
	if (!dumb_mode
		...
		if (dumb_mode)
			...
			SAYF(
					bV bSTOP "   last new path : " cPIN "n/a" cRST " (non-instrumented mode)        ");

		else

			SAYF(
					bV bSTOP "   last new path : " cRST "none yet " cLRD "(odd, check syntax!)      ");
	...
	SAYF(
			bV bSTOP " last uniq crash : " cRST "%-34s " bSTG bV bSTOP " uniq crashes : %s%-6s " bSTG bV "\n",
	...
	SAYF(
			bV bSTOP "  last uniq hang : " cRST "%-34s " bSTG bV bSTOP "   uniq hangs : " cRST "%-6s " bSTG bV "\n",
	...
	SAYF(
			bV bSTOP "  now trying : " cRST "%-21s " bSTG bV bSTOP " favored paths : " cRST "%-22s " bSTG bV "\n",
			stage_name, tmp);
	...
	if (crash_mode) {
		...
		SAYF(
				bV bSTOP " total execs : " cRST "%-21s " bSTG bV bSTOP "   new crashes : %s%-22s " bSTG bV "\n",
				DI(total_execs), unique_crashes ? cLRD : cRST, tmp);

	} else {

		SAYF(
				bV bSTOP " total execs : " cRST "%-21s " bSTG bV bSTOP " total crashes : %s%-22s " bSTG bV "\n",
				DI(total_execs), unique_crashes ? cLRD : cRST, tmp);
	...
	SAYF(
			bV bSTOP "   bit flips : " cRST "%-37s " bSTG bV bSTOP "    levels : " cRST "%-10s " bSTG bV "\n",
			tmp, DI(max_depth));
	...
	SAYF(
			bV bSTOP "  byte flips : " cRST "%-37s " bSTG bV bSTOP "   pending : " cRST "%-10s " bSTG bV "\n",
	...
	SAYF(
			bV bSTOP " arithmetics : " cRST "%-37s " bSTG bV bSTOP "  pend fav : " cRST "%-10s " bSTG bV "\n",
	...
	SAYF(
			bV bSTOP "  known ints : " cRST "%-37s " bSTG bV bSTOP " own finds : " cRST "%-10s " bSTG bV "\n",
	...
	SAYF(
			bV bSTOP "  dictionary : " cRST "%-37s " bSTG bV bSTOP "  imported : " cRST "%-10s " bSTG bV "\n",
	...
	SAYF(" stability : %s%-10s " bSTG bV "\n",
			(stab_ratio < 85 && var_byte_count > 40) ? cLRD : ((queued_variable && (!persistent_mode || var_byte_count > 20)) ? cMGN : cRST),
			tmp);
	...
	SAYF(
			bV bSTOP "        trim : " cRST "%-37s " bSTG bVR bH20 bH2 bH2 bRB "\n" bLB bH30 bH20 bH2 bH bRB bSTOP cRST RESET_G1,
			tmp);
	...
	if (cpu_core_count) {
		...
		if (cpu_aff >= 0) {
			...
			SAYF(SP10 cGRA "[cpu%03u:%s%3u%%" cGRA "]\r" cRST,
					MIN(cpu_aff, 999), cpu_color, MIN(cur_utilization, 999));
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1)
	...
	for (i = 0; i < len - 1; i++) {
		...
		for (j = 1; j <= ARITH_MAX; j++) {
			...
			u16 r1 = orig ^ (orig + j), r2 = orig ^ (orig - j), r3 = orig
					^ SWAP16(SWAP16(orig) + j), r4 = orig
					^ SWAP16(SWAP16(orig) - j);
	...
	for (i = 0; i < len - 1; i++) {
		...
		for (j = 0; j < sizeof(interesting_16) / 2; j++) {
			...
			if (!could_be_bitflip(orig ^ (u16) interesting_16[j])
					...
					&& !could_be_interest(orig, (u16) interesting_16[j], 2,
							0)) {
		...
		for (i = 0; i < use_stacking; i++) {
			...
			case 3:
				...
				if (UR(2)) {
					...
					*(u32*) (out_buf + UR(temp_len - 3)) = interesting_32[UR(
...
static u8 pilot_fuzzing(char **argv) {
	...
	if (!(key_puppet == 0
			&& ((cur_ms_lv - last_path_time < limit_time_puppet)
					|| (last_crash_time != 0
...

	{
		...
		if (key_puppet == 1) {
			if (unlikely(orig_hit_cnt_puppet == 0)) {
				...
				SPLICE_CYCLES_puppet = (UR(
						SPLICE_CYCLES_puppet_up - SPLICE_CYCLES_puppet_low + 1)
...

		{
			...
			for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
				...
				for (i = 0; i < use_stacking; i++) {
					...
					case 10:
						...
						stage_cycles_puppet_v2[swarm_now][STAGE_INTEREST16] +=
								1;
						break;
					...
					case 11:
						...
						stage_cycles_puppet_v2[swarm_now][STAGE_INTEREST32] +=
								1;
						break;
					...
					case 12:
						...
						out_buf[UR(temp_len)] ^= 1 + UR(255);
						stage_cycles_puppet_v2[swarm_now][STAGE_RANDOMBYTE] +=
								1;
						break;
					...
					case 13: {
						...
						temp_len -= del_len;
						stage_cycles_puppet_v2[swarm_now][STAGE_DELETEBYTE] +=
								1;
						break;
					...
					case 14:
						...
						if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
							...
							out_buf = new_buf;
							temp_len += clone_len;
							stage_cycles_puppet_v2[swarm_now][STAGE_Clone75] +=
									1;
						}
					...
					case 15: {
						...
						if (UR(4)) {
							...
							memset(out_buf + copy_to,
									UR(2) ? UR(256) : out_buf[UR(temp_len)],
									copy_len);
						stage_cycles_puppet_v2[swarm_now][STAGE_OverWrite75] +=
								1;
						break;
				...
				if (unlikely(
					...
					u64 temp_temp_puppet = queued_paths + unique_crashes
							- temp_total_found;
			...
			if (use_splicing && splice_cycle++ < SPLICE_CYCLES_puppet
				...
				locate_diffs(in_buf, new_buf, MIN(len, target->len), &f_diff,
						&l_diff);
			...
			abandon_entry: abandon_entry_puppet:
			...
			if (key_puppet == 1) {
				if (unlikely(
						queued_paths + unique_crashes > ((queued_paths + unique_crashes)*limit_time_bound + orig_hit_cnt_puppet))) {
					key_puppet = 0;
					cur_ms_lv = get_cur_time();
					new_hit_cnt = queued_paths + unique_crashes;
			...
			if (unlikely(tmp_pilot_time > period_pilot)) {
				...
				new_hit_cnt = queued_paths + unique_crashes;
				swarm_fitness[swarm_now] = (double) (total_puppet_find
						- temp_puppet_find)
				...
				if (swarm_now == swarm_num) {
					...
					for (i = 0; i < operator_num; i++) {
						...
						core_operator_cycles_puppet_v3[i] =
								core_operator_cycles_puppet[i];
						core_operator_finds_puppet_v2[i] =
								core_operator_finds_puppet[i];
					}
...
void pso_updating(void) {
	...
	for (i = 0; i < operator_num; i++) {
		...
		for (j = 0; j < swarm_num; j++) {
			operator_finds_puppet[i] = operator_finds_puppet[i]
					+ stage_finds_puppet[j][i];
		}
		temp_operator_finds_puppet = temp_operator_finds_puppet
...
EXP_ST void check_binary(u8 *fname) {
  ...
  if (f_data[0] != 0xCF || f_data[1] != 0xFA || f_data[2] != 0xED)
	...
	if (!qemu_mode && !dumb_mode
			&& !memmem(f_data, f_len, SHM_VAL_ENV_VAR, strlen(SHM_VAL_ENV_VAR) + 1)) {
...
static void get_core_count(void) {
  ...
  fclose(f);
    ...
    cur_runnable++;
		...
		OKF(
				"You have %u CPU core%s and %u runnable tasks (utilization: %0.0f%%).",
		...
		case 'L': { /* MOpt mode */
			...
			for (tmp_swarm = 0; tmp_swarm < swarm_num; tmp_swarm++) {
				...
				for (i = 0; i < operator_num; i++) {
					...
					probability_now[tmp_swarm][i] = 0.0;
					x_now[tmp_swarm][i] = ((double) (random() % 7000) * 0.0001
							+ 0.1);
				...
				for (i = 0; i < operator_num; i++) {
					...
					x_now[tmp_swarm][i] = x_now[tmp_swarm][i]
							/ total_puppet_temp;
				}
